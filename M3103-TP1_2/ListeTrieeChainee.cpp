...
void ListeTrieeChainee<TypeInfo>::insereRecWorker(Cellule<TypeInfo>*& ptrCetteListe, const TypeInfo& nouvelleInfo) {
    ...
    if (ptrCetteListe==nullptr)
        ptrCetteListe = new Cellule<TypeInfo>(nouvelleInfo,ptrCetteListe);
    else {
...
template<class TypeInfo>
int ListeTrieeChainee<TypeInfo>::compteNbOccInfoIter (TypeInfo& uneInfo) {
    // retourne le nombre d’occurrences de uneInfo dans cetteListe ChaineeTriee
    return compteNbOccInfoIterWorker(ptrTete, uneInfo);
}

template<class TypeInfo>
int ListeTrieeChainee<TypeInfo>::compteNbOccInfoIterWorker (Cellule<TypeInfo>* ptrCetteListe, TypeInfo& uneInfo) {
    Cellule<TypeInfo>* tmp = ptrCetteListe;
    int nb = 0;
    while(tmp!=nullptr) {
        tmp = tmp->getSuivante();
        if (tmp->getInfo()==uneInfo) {
            nb++;
        }
    }
    return nb;
