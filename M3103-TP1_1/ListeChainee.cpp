...
bool ListeChainee<TypeInfo>::insereAtPositIter(int nouvellePosition, const TypeInfo& nouvelleInfo) {
    ...
    /* RESULTAT
     */
    Cellule<TypeInfo>* temp = ptrTete;
    int pos = 1;
    cout << "meh..." << endl;
    while (pos<=nouvellePosition) {
        if (pos==nouvellePosition) {
            insereTeteWorker(temp,nouvelleInfo);
        } else {
            temp = temp->getSuivante();
            pos++;
        }
    }
    return pos==nouvellePosition;
...
void ListeChainee<TypeInfo>::insereAtPositRecProcWorker(Cellule<TypeInfo>*& ptrCetteListe, int nouvellePosition, const TypeInfo& nouvelleInfo) {
    ...
    /* IMPLANTATION
    Cellule<TypeInfo>*& temp = ptrCetteListe;
    if (temp != nullptr) {
        int pos=0;
        while (temp->getSuivante() != nullptr && pos < nouvellePosition)
    }
     */
...
void ListeChainee<TypeInfo>::afficheGDIterWorker(Cellule<TypeInfo>* ptrListe) const {
    ...
    Cellule<TypeInfo>* temp = ptrListe;
    while (temp != nullptr) {
        temp->affiche();
        cout << " ";
        temp = temp->getSuivante();
...
template<class TypeInfo>
bool ListeChainee<TypeInfo>::estInfoPresenteRecWorker(Cellule<TypeInfo>* ptrListe, TypeInfo& info) {
    if (ptrListe!=nullptr) {
        return (ptrListe->getInfo()==info) || (estInfoPresenteRecWorker(ptrListe->getSuivante(),info));
    } else
        return false;
}

template<class TypeInfo>
bool ListeChainee<TypeInfo>::estInfoPresenteRec(TypeInfo& info) {
    return estInfoPresenteRecWorker(ptrTete, info);
}
