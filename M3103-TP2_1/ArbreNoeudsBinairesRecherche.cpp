...
bool ArbreNoeudBinaireRecherche<TypeInfo>::estInfoPresenteWorker(NoeudBinaire<TypeInfo>* ptrRac,
    ...
    if (ptrRac==nullptr)
        ...
        if (ptrRac->getInfo()==infoCible)
            ...
            return estInfoPresenteWorker(ptrRac->getPtrFilsDroit(), infoCible) || estInfoPresenteWorker(ptrRac->getPtrFilsGauche(), infoCible);
    }
...
bool ArbreNoeudBinaireRecherche<TypeInfo>::aMemeGeometrieQueWorker(const NoeudBinaire<TypeInfo>* monPtrRac, const NoeudBinaire<TypeInfo>* sonPtrRac) const {
    ...
    if ((monPtrRac==nullptr)&&(sonPtrRac==nullptr))
        ...
        if ((monPtrRac==nullptr)||(sonPtrRac==nullptr))
            ...
            return aMemeGeometrieQueWorker(monPtrRac->getPtrFilsGauche(),sonPtrRac->getPtrFilsGauche()) && aMemeGeometrieQueWorker(monPtrRac->getPtrFilsDroit(),sonPtrRac->getPtrFilsDroit());
    }
}
...
int ArbreNoeudBinaireRecherche<TypeInfo>::getNombreOccurrencesWorker(const NoeudBinaire<TypeInfo>* ptrRac, const TypeInfo& uneInfo) const {
    ...
    if (ptrRac==nullptr)
        ...
        if (ptrRac->getInfo()==uneInfo)
            ...
            return getNombreOccurrencesWorker(ptrRac->getPtrFilsGauche(),uneInfo)+getNombreOccurrencesWorker(ptrRac->getPtrFilsDroit(),uneInfo);
    }
